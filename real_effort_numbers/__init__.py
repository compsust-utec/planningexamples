...
class Constants(BaseConstants):
    ...
    num_rounds = 2
    payment_per_correctanswer = cu(1)
...
class AddNumbers(Page):
    ...
    def before_next_page(player: Player, timeout_happened):
        if player.sum_of_numbers == player.number_entered:
            player.payoff = Constants.payment_per_correctanswer
...
class CombinedResults(Page):
    @staticmethod
    # this is done so that the page is not displayed unless the last round has been completed
